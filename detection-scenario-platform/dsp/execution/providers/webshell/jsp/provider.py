"""JspWebshellProvider — JSP family adapter with real execution via runtime."""
...
class JspWebshellProvider(GenericWebshellProvider):
    """JSP webshell provider — delegates execution to ``JspWebshellRuntime``."""

    provider_type = "jsp"
    provider_name = "JSP Webshell Provider"
    session_class = JspProviderSession
    provider_version = JSP_PROVIDER_VERSION
    ...
    def create_runtime(
        ...
        *,
        ...
        """Build a JSP runtime bound to this provider's transport and URL."""
        if self._transport is None:
            raise JspProviderNotReadyError(
                "transport is required to create JSP runtime",
                field="transport",
            )
        if config is None:
    ...
    def connect(self) -> None:
        ...
        runtime = self._require_jsp_runtime()
    ...
    def execute_command(
        ...
        *,
        ...
        """Execute a command through the JSP webshell transport path."""
    ...
    def _require_jsp_runtime(self) -> JspWebshellRuntime:
        runtime = self._runtime
        if runtime is None:
            raise JspProviderNotReadyError(
                "JSP runtime is not attached; call create_runtime() first",
                field="runtime",
            )
        if not isinstance(runtime, JspWebshellRuntime):
            raise JspProviderNotReadyError(
                "attached runtime must be JspWebshellRuntime",
                field="runtime",
            )
        return runtime
