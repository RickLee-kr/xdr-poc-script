"""PHP webshell execution tests — mocked backend only, no live network."""
...
def _ok(body: bytes = b"php-output", **headers: str) -> HttpBackendResponse:
...
def test_php_execute_get_cmd_param():
    backend = RecordingHttpBackend(responses=[_ok(b"www-data\n")])
    ...
    assert result.execution_metadata["php_command_param"] == "cmd"


def test_php_execute_post_cmd_form():
...
def test_response_passthrough_without_parsing():
    ...
    assert metadata["transport_response_size"] == len(b"www-data\nuid=33")
    ...
    assert "www-data" not in str(metadata)
...
def test_cleanup_invocation():
    ...
    assert isinstance(runtime, PhpWebshellRuntime)
    provider.cleanup()
    ...
    backend = RecordingHttpBackend(
        responses=[
            WebshellTransportTimeoutError(
                ...
                url="https://lab.example/shell.php",
...
def test_execute_without_runtime_raises():
    ...
    with pytest.raises(PhpProviderNotReadyError, match="create_runtime"):
        provider.execute_command("id")


def test_no_success_inference():
    source = inspect.getsource(PhpWebshellRuntime.execute_command)
...
def test_no_stdout_stderr_parsing_in_runtime():
    source = inspect.getsource(
        __import__(
            "dsp.execution.providers.webshell.php.php_runtime",
            fromlist=["PhpWebshellRuntime"],
        )
    )
    lowered = source.lower()
...
def test_php_command_encoder_format():
    encoder = PhpCommandEncoder()
...
def test_transport_request_preserves_response_headers():
    ...
    response = runtime.transport.send_get(
        TransportRequest(url="https://lab.example/shell.php")
    )
