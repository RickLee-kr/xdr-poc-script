"""ASPX webshell execution tests — mocked backend only, no live network."""
...
def _ok(body: bytes = b"aspx-output", **headers: str) -> HttpBackendResponse:
...
def test_aspx_execute_get_cmd_param():
    backend = RecordingHttpBackend(responses=[_ok(b"iis apppool\\defaultapppool\n")])
    ...
    assert result.execution_metadata["aspx_command_param"] == "cmd"


def test_aspx_execute_post_cmd_form():
...
def test_response_passthrough_without_parsing():
    backend = RecordingHttpBackend(
        responses=[_ok(b"nt authority\\iis apppool\\defaultapppool", x_exit="0")]
    )
    provider = _connected_provider(backend)
    result = provider.execute_command("id")
    metadata = result.execution_metadata
    assert metadata["transport_response_size"] == len(
        b"nt authority\\iis apppool\\defaultapppool"
    )
    ...
    assert "iis apppool" not in str(metadata)
...
def test_cleanup_invocation():
    ...
    assert isinstance(runtime, AspxWebshellRuntime)
    provider.cleanup()
    ...
    backend = RecordingHttpBackend(
        responses=[
            WebshellTransportTimeoutError(
                ...
                url="https://lab.example/shell.aspx",
...
def test_execute_without_runtime_raises():
    ...
    with pytest.raises(AspxProviderNotReadyError, match="create_runtime"):
        provider.execute_command("id")


def test_no_success_inference():
    source = inspect.getsource(AspxWebshellRuntime.execute_command)
...
def test_no_stdout_stderr_parsing_in_runtime():
    source = inspect.getsource(
        __import__(
            "dsp.execution.providers.webshell.aspx.aspx_runtime",
            fromlist=["AspxWebshellRuntime"],
        )
    )
    lowered = source.lower()
...
def test_aspx_command_encoder_format():
    encoder = AspxCommandEncoder()
...
def test_transport_request_preserves_response_headers():
    ...
    response = runtime.transport.send_get(
        TransportRequest(url="https://lab.example/shell.aspx")
    )
