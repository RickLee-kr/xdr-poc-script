"""Kerberos failure Path Equality tests — Store-only validation and reporting."""
...
def test_kerberos_failure_path_equality():
    store = EventStore(":memory:")
    run_id = "kerberos_failure_pe_run"
    store.open_run(run_id)
    _append_lifecycle(store, run_id, "kerberos_failure")

    artifact = "administrator@LOCAL.REALM@10.10.10.30:88"
    for seq in (1, 2, 3):
        store.append(
            build_kerberos_auth_attempt_event(
                run_id=run_id,
    ...
    result = validator.validate(run_id, "kerberos_failure")
    ...
    assert result.metrics["kerberos_auth_attempt_count"] == 3
    assert result.metrics["kerberos_auth_failed_count"] == 3
    assert table[0]["metrics"] == result.metrics
    assert report.traffic_validation[0].metrics == result.metrics


def test_kerberos_failure_empty_traffic_code_failure():
    store = EventStore(":memory:")
    run_id = "kerberos_failure_empty"
    store.open_run(run_id)
    _append_lifecycle(store, run_id, "kerberos_failure")

    loader = PluginLoader()
    registry = loader.discover_and_load()
    result = ValidationEngine(store, registry).validate(run_id, "kerberos_failure")
