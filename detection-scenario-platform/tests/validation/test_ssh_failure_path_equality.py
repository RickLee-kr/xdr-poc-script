"""SSH login failure Path Equality tests — Store-only validation and reporting."""
...
def test_ssh_failure_path_equality():
    store = EventStore(":memory:")
    run_id = "ssh_failure_pe_run"
    store.open_run(run_id)
    _append_lifecycle(store, run_id, "ssh_failure")

    artifact = "admin@10.10.10.20:22"
    for seq in (1, 2, 3):
        store.append(
            build_ssh_auth_attempt_event(
                run_id=run_id,
                ...
                artifact=artifact,
                source="dry_run",
                evidence={"seq": seq, "username": "admin"},
            )
        )
        store.append(
            build_ssh_auth_failed_event(
    ...
    result = validator.validate(run_id, "ssh_failure")
    ...
    assert result.metrics["ssh_auth_attempt_count"] == 3
    assert result.metrics["ssh_auth_failed_count"] == 3
    assert table[0]["metrics"] == result.metrics
    assert report.traffic_validation[0].metrics == result.metrics


def test_ssh_failure_empty_traffic_code_failure():
    store = EventStore(":memory:")
    run_id = "ssh_failure_empty"
    store.open_run(run_id)
    _append_lifecycle(store, run_id, "ssh_failure")

    loader = PluginLoader()
    registry = loader.discover_and_load()
    result = ValidationEngine(store, registry).validate(run_id, "ssh_failure")
