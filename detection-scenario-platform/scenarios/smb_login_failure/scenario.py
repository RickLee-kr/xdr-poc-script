"""SMB login failure scenario — safe authentication failure simulation."""
...
from dsp.protocols.smb.smb_events import SMB_LOGIN_FAILURE_TRAFFIC_EVENTS


def _load_executor():
    path = Path(__file__).with_name("executor.py")
    spec = importlib.util.spec_from_file_location("smb_login_failure_executor", path)
...
class SmbLoginFailureScenario(Scenario):
    @classmethod
    def scenario_id(cls) -> str:
        return "smb_login_failure"
    ...
    def execute(self, ctx: RunContext, targets: TargetSet) -> None:
        ...
        params = dict(ctx.config.scenario_params.get("smb_login_failure", {}))
    ...
    def summarize(self, ctx: RunContext) -> ScenarioSummary:
        ...
        return ScenarioSummary(
            scenario_id=scenario_id,
            metrics={
                "smb_auth_attempt_count": attempts,
                "smb_auth_failed_count": failures,
                "total_events": total,
            },
            event_count=total,
            notes=[
                f"SMB Login Failure — mode={mode}, ports 445/139, safe_mode=true",
                f"Traffic events: {', '.join(sorted(SMB_LOGIN_FAILURE_TRAFFIC_EVENTS))}",
