"""Kerberos failure scenario — safe authentication failure simulation."""
...
from dsp.protocols.kerberos.kerberos_events import KERBEROS_FAILURE_TRAFFIC_EVENTS


def _load_executor():
    path = Path(__file__).with_name("executor.py")
    spec = importlib.util.spec_from_file_location("kerberos_failure_executor", path)
...
class KerberosFailureScenario(Scenario):
    @classmethod
    ...
    def execute(self, ctx: RunContext, targets: TargetSet) -> None:
        ...
        params = dict(ctx.config.scenario_params.get("kerberos_failure", {}))
    ...
    def summarize(self, ctx: RunContext) -> ScenarioSummary:
        ...
        return ScenarioSummary(
            scenario_id=scenario_id,
            metrics={
                "kerberos_auth_attempt_count": attempts,
                "kerberos_auth_failed_count": failures,
                "total_events": total,
            },
            event_count=total,
            notes=[
                f"Kerberos Failure — mode={mode}, port 88/udp, safe_mode=true",
                f"Traffic events: {', '.join(sorted(KERBEROS_FAILURE_TRAFFIC_EVENTS))}",
